export const en = {
	"menu.functions": "Quick Functions",
	"menu.screenshot": "Screenshot",
	"menu.settings": "Settings",
	"menu.settings.generalSettings": "Interface Settings",
	"menu.settings.hotKeySettings": "Hotkey Settings",
	"menu.settings.systemSettings": "System Settings",
	"menu.tools": "Toolbox",
	"menu.tools.translation": "Translation",
	"settings.commonSettings": "Common Settings",
	"settings.language": "Language",
	"settings.darkMode": "Dark Mode",
	"settings.controlNode": "Selection Control Style",
	"settings.controlNode.circle": "Circle",
	"settings.controlNode.polyline": "Polyline",
	"settings.screenshotSettings": "Screenshot Settings",
	"settings.findChildrenElements": "Find Window Child Elements",
	"settings.performanceMode": "Performance Mode",
	"settings.performanceMode.tip1":
		"Enabling Performance Mode will attempt to reduce rendering quality to improve performance (keep enabled during rapid software iteration)",
	"settings.drawingHotKey": "Drawing Hotkeys",
	"settings.addKeyConfig": "Add Key Configuration",
	"settings.pleasePressTheKey": "Please press a key",
	"settings.keyConfig": 'Key Configuration for "{title}"',
	"settings.resetSettings":
		'Are you sure you want to reset "{title}" to default settings?',
	"settings.antialias": "Antialias",
	"settings.renderSettings": "Render Settings",
	"settings.drawLine": "Draw Line",
	"settings.enableDrawLineSimplify": "Enable Line Simplification",
	"settings.enableDrawLineSimplify.tip":
		"Reduces line sampling points to minimize jitter during drawing, but may decrease angle accuracy",
	"settings.drawLineSimplifyTolerance": "Simplification Tolerance",
	"settings.drawLineSimplifyTolerance.tip":
		"Lower tolerance preserves more details",
	"settings.drawLineSimplifyHighQuality": "High-Quality Simplification",
	"settings.drawLineSimplifyHighQuality.tip":
		"Improves quality but significantly impacts performance",
	"settings.enableDrawLineSmooth": "Enable Line Smoothing",
	"settings.enableDrawLineSmooth.tip": "Smooths lines to reduce jagged edges",
	"settings.drawLineSmoothRatio": "Subdivision Ratio",
	"settings.drawLineSmoothRatio.tip": "Lower ratio produces smoother lines",
	"settings.drawLineSmoothIterations": "Subdivision Iterations",
	"settings.drawLineSmoothIterations.tip":
		"More iterations create smoother lines",
	"settings.systemSettings.commonSettings": "Common Settings",
	"settings.systemSettings.commonSettings.autoStart": "Launch at Startup",
	"settings.hotKeySettings.translation": "Translation Hotkey",
	"settings.hotKeySettings.keyEventTooltip": "{message} ({key})",
	"settings.chatSettings.maxTokens": "Max Tokens",
	"settings.chatSettings.maxTokens.tip":
		"Maximum number of tokens the model will generate in a response",
	"settings.chatSettings.temperature": "Temperature",
	"settings.chatSettings.temperature.tip":
		"Higher values (e.g., 0.8) make outputs more random; lower values (e.g., 0.2) make them focused/deterministic",
	"home.commonFunction": "Common Features",
	"home.screenshotFunction": "Screenshot Feature",
	"home.translationFunction": "Translation Feature",
	"home.translationSelectText": "Translate Selected Text",
	"home.translation": "Translation",
	"home.screenshot": "Screenshot",
	"home.shortcut": "Hotkey",
	"home.shortcut.registered": "Registered successfully",
	"home.shortcut.unregistered":
		"Registration interrupted (possible hotkey conflict)",
	"home.shortcut.error": "Registration failed (hotkey occupied by another app)",
	"home.exit": "Exit",
	"draw.drag": "Drag",
	"draw.red": "Red",
	"draw.orange": "Orange",
	"draw.green": "Green",
	"draw.blue": "Blue",
	"draw.purple": "Purple",
	"draw.black": "Black",
	"draw.gray": "Gray",
	"draw.white": "White",
	"draw.radiusDesc": "Radius: {radius}px",
	"draw.fillShape": "Fill",
	"draw.enableBlur": "Blur Effect",
	"draw.regionBlur": "Area Blur",
	"draw.selectRect": "Select Rectangle",
	"draw.fontSize": "Font Size",
	"draw.fontFamily": "Font Family",
	"draw.bold": "Bold",
	"draw.italic": "Italic",
	"draw.underline": "Underline",
	"draw.strikethrough": "Strikethrough",
	"draw.stroke": "Stroke",
	"draw.fontFamily.default": "Default",
	"draw.enableRadius": "Rounded Corners",
	"draw.autoSelectWindowElement": "Auto-Select Window Elements",
	"draw.autoSelectWindowElement.loading": "Loading",
	"draw.autoSelectWindowElement.loaded": "Loaded",
	"draw.keyEventTooltip": "{message} (Hotkey: {key})",
	"draw.cancel.tip1": "Confirm to cancel screenshot?",
	"draw.tabKey": "Tab",
	"draw.selectWindowOrElement": "Switch Window/Element Selection",
	"draw.mouseWheel": "Mouse Wheel",
	"draw.changeSelectLevel": "Change Selection Level",
	"draw.rgb": "RGB ({r}, {g}, {b})",
	"draw.moveTool": "Move",
	"draw.selectTool": "Select",
	"draw.rectTool": "Rectangle",
	"draw.diamondTool": "Diamond",
	"draw.ellipseTool": "Ellipse",
	"draw.arrowTool": "Arrow",
	"draw.lineTool": "Line",
	"draw.penTool": "Pen",
	"draw.highlightTool": "Highlight",
	"draw.mosaicTool": "Mosaic",
	"draw.blurTool": "Blur",
	"draw.textTool": "Text",
	"draw.eraserTool": "Eraser",
	"draw.undoTool": "Undo",
	"draw.redoTool": "Redo",
	"draw.redoUndoTool": "Redo/Undo",
	"draw.cancelTool": "Cancel",
	"draw.copyTool": "Copy to Clipboard",
	"draw.saveTool": "Save as File",
	"draw.close": "Close",
	"draw.ocrDetectTool": "Text Recognition",
	"draw.fixedTool": "Pin to Screen",
	"draw.lockWidthHeightPicker": "Toggle Lock Aspect Ratio",
	"draw.lockAnglePicker": "Toggle Angle Lock",
	"draw.removeTool": "Delete Selected Element",
	"draw.colorPickerCopy": "Copy Color",
	"draw.colorPickerMoveUp": "Move Up",
	"draw.colorPickerMoveDown": "Move Down",
	"draw.colorPickerMoveLeft": "Move Left",
	"draw.colorPickerMoveRight": "Move Right",
	"draw.penDrawLine": "Draw Line",
	"draw.shiftKey": "Shift",
	"draw.enableStraightLinePicker": "Toggle Straight Line Drawing",
	"draw.resizeFromCenterPicker": "Resize from Center",
	"draw.maintainAspectRatioPicker": "Maintain Aspect Ratio",
	"draw.rotateWithDiscreteAnglePicker": "Fixed Angle Rotation",
	"draw.autoAlignPicker": "Auto Align",
	"draw.copySelectedText": "Copy Selected Text",
	"draw.positionX": "Position X",
	"draw.positionY": "Position Y",
	"draw.width": "Width",
	"draw.height": "Height",
	"draw.lockAspectRatio": "Lock Aspect Ratio",
	"draw.radius": "Radius",
	"draw.shadowWidth": "Shadow Width",
	"draw.shadowColor": "Shadow Color",
	"draw.resizeModal": "Resize",
	"draw.confirm": "Confirm",
	"tools.translation": "Translation",
	"tools.translation.sourceLanguage": "Source Language",
	"tools.translation.targetLanguage": "Target Language",
	"tools.translation.language.auto": "Auto Detect",
	"tools.translation.language.english": "English",
	"tools.translation.language.simplifiedChinese": "Simplified Chinese",
	"tools.translation.language.traditionalChinese": "Traditional Chinese",
	"tools.translation.language.spanish": "Spanish",
	"tools.translation.language.french": "French",
	"tools.translation.language.arabic": "Arabic",
	"tools.translation.language.german": "German",
	"tools.translation.language.italian": "Italian",
	"tools.translation.language.portuguese": "Portuguese",
	"tools.translation.language.russian": "Russian",
	"tools.translation.language.turkish": "Turkish",
	"tools.translation.language.japanese": "Japanese",
	"tools.translation.placeholder": "Enter text to translate",
	"tools.translation.copyAndHide": "Copy and Hide",
	"tools.translation.copy": "Copy",
	"tools.translation.replace": "Replace",
	"tools.translation.getSelectText.loading": "Retrieving selected text",
	"tools.translation.getSelectText.failed": "Failed to retrieve",
	"tools.translation.requestTimeout":
		"Local translation model timed out. Check that the service is running or increase the translation timeout.",
	"tools.chat.placeholder": "Send a message",
	"tools.chat.welcome.title": "👋 Hello, how can I help you?",
	"tools.chat.welcome.description":
		"I'm an AI assistant. Feel free to ask me anything!",
	"tools.chat.requestAborted": "Request aborted",
	"tools.chat.requestFailed": "Request failed, please try again",
	"tools.chat.agentPlaceholder": "Generating content...",
	"tools.chat.newSession.tip": "New session created",
	"tools.chat.session.current": "Current",
	"tools.chat.sessions": "Sessions",
	"tools.chat.newSession": "New Session",
	"tools.chat.session.clear": "Clear All Sessions",
	"tools.chat.session.clear.tip": "Confirm to clear all sessions?",
	"tools.chat.reasoner": "Supports reasoning",
	"tools.chat.thinking": "Reasoning mode",
	"tools.chat.thinking.unsupported":
		"The selected model does not support reasoning mode",
	"tools.chat.custom": "Custom",
	"tools.translation.type": "Translation Service",
	"tools.translation.domain": "Translation Domain",
	"tools.translation.type.youdao": "Youdao Translate",
	"tools.translation.type.deepseek": "DeepSeek",
	"tools.translation.type.openAiCompatible": "AI / OpenAI Compatible",
	"tools.translation.type.deepl": "DeepL Translate",
	"tools.translation.domain.general": "General",
	"tools.translation.domain.computers": "Computers",
	"tools.translation.domain.medicine": "Medicine",
	"tools.translation.domain.finance": "Finance",
	"tools.translation.domain.game": "Gaming",
	"settings.systemSettings.networkSettings": "Network Settings",
	"settings.systemSettings.networkSettings.proxy": "System Proxy",
	"settings.chatSettings.thinkingBudgetTokens": "Thinking Budget Tokens",
	"settings.chatSettings.thinkingBudgetTokens.tip":
		'For reasoning modes like Claude models, the thinking budget determines the maximum number of tokens for their internal reasoning process. Note: The thinking budget tokens cannot be less than "Max Tokens", therefore if this value is effective, the "Max Tokens" may be determined by the server',
	"about.title": "Snow Shot",
	"about.subtitle":
		"A tool software designed with simplicity and elegance as the concept ❄️",
	"about.version": "Version",
	"about.author": "Original author: mg-chao",
	"about.license.title": "📄 Open Source License",
	"about.license.description": "This project adopts a dual licensing model:",
	"about.license.nonCommercial": "Non-commercial scenarios:",
	"about.license.nonCommercialType": "Apache v2.0 License applies",
	"about.license.commercial": "Commercial scenarios:",
	"about.license.commercialType": "GPL v3 License applies",
	"about.feedback.title": "Feedback",
	"about.feedback.github": "GitHub Issues",
	"common.edit": "Edit",
	"common.delete": "Delete",
	"common.delete.tip": 'Are you sure you want to delete "{name}"?',
	"common.newVersion":
		"Snow Shot version {latestVersion} has been released \nCurrent version {currentVersion}",
	"common.newVersion.title": "New Version {latestVersion} Available",
	"common.newVersion.updateReady": "Update Downloaded",
	"common.newVersion.updateReady.description":
		"Snow Shot {version} is ready. Would you like to restart the app now to install the update?",
	"common.newVersion.updateNow": "Update Now",
	"common.newVersion.updateLater": "Later",
	"common.newVersion.downloading": "Downloading update...",
	"common.newVersion.check": "Check for updates",
	"common.newVersion.latest": "You're already up to date",
	"common.newVersion.updateFailed": "Update failed: {error}",
	"common.newVersion.unavailable":
		"Online updates are unavailable for this installation channel. Install a release from GitHub manually.",
	"common.permission.error.title": "Missing Critical Permissions",
	"common.permission.error.description":
		'Please enable necessary permissions for "Screen Recording & System Audio" and "Accessibility"',
	"common.permission.error.goToSettings": "Go to Settings",
	"settings.functionSettings.videoRecordSettings.keyDisplaySettings":
		"Key Display",
	"settings.functionSettings.videoRecordSettings.keyDisplayFontSize":
		"Font Size",
	"settings.functionSettings.videoRecordSettings.keyDisplayBackgroundColor":
		"Background Color",
	"settings.functionSettings.videoRecordSettings.keyDisplayTextColor":
		"Text Color",
	"settings.functionSettings.videoRecordSettings.keyDisplayDuration":
		"Duration",
	"settings.functionSettings.videoRecordSettings.keyDisplayMergeDuration":
		"Merge Time",
	"settings.functionSettings.videoRecordSettings.keyDisplayDirection":
		"Direction",
	"settings.functionSettings.videoRecordSettings.keyDisplayDirection.horizontal":
		"Horizontal",
	"settings.functionSettings.videoRecordSettings.keyDisplayDirection.vertical":
		"Vertical",
	"settings.functionSettings.translationSettings.apiConfig.apiType.openAiCompatible":
		"AI / OpenAI Compatible",
	"settings.functionSettings.translationSettings.apiConfig.apiType.deepL":
		"DeepL",
	"settings.functionSettings.translationSettings.apiConfig.modelName":
		"Service Name",
	"settings.functionSettings.translationSettings.apiConfig.modelName.tip":
		"Name shown in the translation service selector",
	"settings.functionSettings.translationSettings.apiConfig.modelName.required":
		"Enter a service name",
	"settings.functionSettings.translationSettings.apiConfig.apiModel":
		"Model Name",
	"settings.functionSettings.translationSettings.apiConfig.apiModel.tip":
		"Model name used by the OpenAI-compatible endpoint",
	"settings.functionSettings.translationSettings.apiConfig.apiModel.required":
		"Enter a model name",
	"settings.functionSettings.ocrSettings.glmOcrApiConfig.modelName":
		"GLM OCR Service Name",
	"settings.functionSettings.ocrSettings.glmOcrApiConfig.apiUri":
		"GLM OCR API URL",
	"settings.functionSettings.ocrSettings.glmOcrApiConfig.apiUri.tip":
		"Configure the GLM OCR service URL and port here. Rapid OCR does not use this URL and has no service port.",
	"settings.functionSettings.ocrSettings.glmOcrApiConfig.apiKey":
		"GLM OCR API Key",
	"settings.functionSettings.ocrSettings.glmOcrApiConfig.apiModel":
		"GLM OCR Model Name",
	"settings.functionSettings.ocrSettings.rapidOcrRuntime.v4.title":
		"Rapid OCR V4 (Compatibility Fallback)",
	"settings.functionSettings.ocrSettings.rapidOcrRuntime.v4.description":
		"Runs local ONNX inference, does not call the GLM OCR API, and has no service port. V4 and V5 share the same detection/orientation pipeline; the main difference is the recognition model. V4 uses PP-OCRv4 recognition and is best kept as a stable compatibility or old-result comparison fallback. Prefer V5 for daily use.",
	"settings.functionSettings.ocrSettings.rapidOcrRuntime.v5.title":
		"Rapid OCR V5 (Recommended)",
	"settings.functionSettings.ocrSettings.rapidOcrRuntime.v5.description":
		"Runs local ONNX inference, does not call the GLM OCR API, and has no service port. V5 and V4 share the same detection/orientation pipeline; the main difference is the recognition model. V5 uses PP-OCRv5 recognition and is preferred for daily screenshots, text copying, and mixed Chinese/English text. Fall back to V4 only when compatibility issues appear.",
	"settings.functionSettings.ocrSettings.visionSettings":
		"Vision Understanding (Not Screenshot OCR)",
	"settings.functionSettings.ocrSettings.htmlVisionModel":
		"Image-to-HTML Vision Model",
	"settings.functionSettings.ocrSettings.htmlVisionModel.tip":
		"Used only for image-to-HTML/Markdown vision features. It does not decide the screenshot OCR backend; screenshot OCR uses the Text Recognition Model selection.",
	"plugin.glm_ocr.name": "GLM OCR",
	"plugin.glm_ocr.description":
		"Enable GLM OCR text recognition through an OpenAI-compatible endpoint. Enter the service URL, port, and model in Function Settings before use.",
	"plugin.glm_ocr.functionDescription": "API Text Recognition",
	"plugin.rapid_ocr.name": "Rapid OCR Local Models",
	"plugin.rapid_ocr.description":
		"Install Rapid OCR V4/V5 ONNX models. When Rapid OCR V4/V5 is selected, recognition runs inside the local Snow Shot process without starting a separate service or using a port.",
	"plugin.rapid_ocr.functionDescription": "Local Text Recognition",
	"plugin.translate.functionDescription": "Translation",
	"settings.systemSettings.screenshotSettings.ocrModel":
		"Text Recognition Model",
	"settings.systemSettings.screenshotSettings.ocrModel.tip":
		"The selected item is the backend used by screenshot OCR. GLM OCR calls an OpenAI-compatible API service configured by the user. Rapid OCR V4/V5 loads ONNX models inside the local Snow Shot process and does not start a separate service or use a port.",
	"settings.systemSettings.screenshotSettings.ocrModel.glmOcr":
		"GLM OCR (API Service)",
	"settings.systemSettings.screenshotSettings.ocrModel.rapidOcrV4":
		"Rapid OCR V4 (Local ONNX)",
	"settings.systemSettings.screenshotSettings.ocrModel.rapidOcrV5":
		"Rapid OCR V5 (Local ONNX)",
};
